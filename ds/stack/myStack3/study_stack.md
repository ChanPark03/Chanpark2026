...
## 7. 추가: 구조체 멤버 접근 연산자 (`.` 와 `->`)

코드에서 `s1.size` 와 `ps->size` 두 가지 형태를 모두 보셨을 겁니다. 둘 다 구조체의 멤버에 접근하는 연산자이지만, 사용하는 대상이 다릅니다.

### 1. `.` (점) 연산자

-   **언제 사용하나요?** 구조체 **변수** 자체에서 멤버에 접근할 때 사용합니다.
-   **예시:**
    ```c
    Stack s1;       // s1은 Stack 타입의 '변수'입니다.
    s1.size = 10;   // s1 변수 안의 size 멤버에 10을 대입합니다.
    ```

### 2. `->` (화살표) 연산자

-   **언제 사용하나요?** 구조체를 가리키는 **포인터**를 통해 멤버에 접근할 때 사용합니다.
-   **예시:**
    ```c
    Stack s1;
    Stack *ps = &s1;  // ps는 Stack 타입을 가리키는 '포인터'입니다. s1의 주소를 담고 있죠.

    ps->size = 10;    // ps 포인터가 가리키는 곳(즉, s1)의 size 멤버에 10을 대입합니다.
    ```
-   `ps->size`는 `(*ps).size` 와 완전히 동일한 표현입니다. 포인터가 가리키는 주소로 먼저 이동(`*ps`)한 뒤, 그곳에 있는 구조체 변수의 멤버(`.size`)에 접근한다는 의미인데, 이 과정을 `->` 연산자 하나로 줄여 쓸 수 있어 편리합니다.

`push(&s1)`처럼 함수의 인자로 구조체 변수의 주소(`&s1`)를 넘겨주었기 때문에, `push` 함수 내에서는 매개변수인 포인터 `ps`를 통해 `->` 연산자로 멤버에 접근했던 것입니다.
